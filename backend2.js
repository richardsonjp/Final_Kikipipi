...
app.post('/invoice/dates',(req,res)=>{
    db.query(`select * from invoicesdetail where year(time)= ${req.body.year} and month(time)=${req.body.month} and day(time)=${req.body.day} `,
    (err,result)=>{
        if (err) throw err;
...
app.get('/invoice/searchby',(req,res)=>{
    db.query(`select id_inv, product_name, quantity, total_price, time , no_telp , nama_pembeli from invoicesdetail dtl join invoicedata dt on dtl.inv_id = dt.id_inv`,
    (err,result)=>{
        if (err) throw err;
...
// app.post('/search/:idtype', (req,res)=>{
